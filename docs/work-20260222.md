...
> 본 문서는 Telegram UX 전환 중심 과거 작업 기록입니다.
> 현재 제품 기준 문서는 `docs/overhaul-20260302.md`입니다.
...
- Linear 업데이트 안정화(추가):
  - `linear_update_issue` GraphQL mutation 변수 형태를 Linear 스키마에 맞게 수정
    - 기존: `issueUpdate(input: { id, ... })`
    - 수정: `issueUpdate(id: $id, input: {...})`
  - 동일 실패 재발 시 원인 파악을 위해 validation 메시지에 실패 필드 힌트(`description` 등) 노출 보강
