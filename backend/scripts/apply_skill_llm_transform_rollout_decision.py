...
ALLOWED_ENV_KEYS = {
    "SKILL_LLM_TRANSFORM_PIPELINE_ENABLED",
    "SKILL_LLM_TRANSFORM_PIPELINE_SHADOW_MODE",
    "SKILL_LLM_TRANSFORM_PIPELINE_TRAFFIC_PERCENT",
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply skill+llm transform rollout decision JSON to .env file")
    parser.add_argument(
        "--from-json",
        type=str,
        default="../docs/reports/skill_llm_transform_rollout_decision_latest.json",
        help="Path to JSON generated by decide_skill_llm_transform_rollout.py",
    )
    parser.add_argument(
        "--env-file",
        type=str,
    ...
    if not suggested_env:
        print("[apply-skill-llm-transform-rollout] no suggested_env found")
        return 0

    lines = _read_env_lines(env_path)
    result = _apply_to_lines(lines, suggested_env)

    print("[apply-skill-llm-transform-rollout] preview")
    ...
    if not args.apply:
        print("[apply-skill-llm-transform-rollout] dry-run complete (use --apply to write)")
        return 0

    _write_env_lines(env_path, lines)
    print("[apply-skill-llm-transform-rollout] applied")
    return 0
