...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply autonomous rollout decision JSON to .env file")
    parser.add_argument(
        "--from-json",
        type=str,
        default="../docs/reports/autonomous_rollout_decision_latest.json",
        help="Path to JSON generated by decide_autonomous_rollout.py",
    )
    parser.add_argument(
        "--env-file",
        type=str,
    ...
    if not suggested_env:
        print("[apply-autonomous-rollout] no suggested_env found")
        return 0

    lines = _read_env_lines(env_path)
    result = _apply_to_lines(lines, suggested_env)

    print("[apply-autonomous-rollout] preview")
    ...
    if not args.apply:
        print("[apply-autonomous-rollout] dry-run complete (use --apply to write)")
        return 0

    _write_env_lines(env_path, lines)
    print("[apply-autonomous-rollout] applied")
    return 0
