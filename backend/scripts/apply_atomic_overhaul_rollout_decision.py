...
ALLOWED_ENV_KEYS = {
    "ATOMIC_OVERHAUL_ENABLED",
    "ATOMIC_OVERHAUL_SHADOW_MODE",
    "ATOMIC_OVERHAUL_TRAFFIC_PERCENT",
    "ATOMIC_OVERHAUL_ALLOWLIST",
    "ATOMIC_OVERHAUL_LEGACY_FALLBACK_ENABLED",
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply Atomic Overhaul rollout decision JSON to .env file")
    parser.add_argument(
        "--from-json",
        type=str,
        default="../docs/reports/atomic_overhaul_rollout_decision_latest.json",
        help="Path to JSON generated by decide_atomic_overhaul_rollout.py",
    )
    parser.add_argument("--env-file", type=str, default=".env", help="Target env file path")
    ...
    if not suggested_env:
        print("[apply-atomic-rollout] no suggested_env found")
        return 0

    lines = _read_env_lines(env_path)
    result = _apply_to_lines(lines, suggested_env)

    print("[apply-atomic-rollout] preview")
    ...
    if not args.apply:
        print("[apply-atomic-rollout] dry-run complete (use --apply to write)")
        return 0

    _write_env_lines(env_path, lines)
    print("[apply-atomic-rollout] applied")
    return 0
